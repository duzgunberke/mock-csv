...
def maas_hesapla(rol_kategori, kidem, deneyim_yil, sehir):
    ...
    deneyim_carpani = 1.0 + (deneyim_yil * 0.01)  
...
def veri_seti_olustur(n=10000):
    ...
    for i in range(n):
        ...
        is_baslama_gunu = random.randint(1, 28) 
        ...
        teknik_baz = 40 + min(deneyim * 4, 40)  
        ...
        if deneyim < 1:
            ...
            izin_gun = 14 + int(deneyim)  
        elif deneyim < 15:
            izin_gun = 20 + int((deneyim - 5) * 0.5)  
        else:
            izin_gun = 26  
        ...
        if random.random() < terfi_olasiliği:
            max_terfi_ay = min(int(deneyim * 12 * 0.7), 60)  
